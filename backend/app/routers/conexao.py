...
async def buscar_cpe(
    ...
    if not resposta.success:
        raise HTTPException(status_code=400, detail="Não foi possível buscar os CPEs.")
...
async def sessao_online_cpe(cpe_pk: int, ctx: AuthContext = Depends(get_auth_context)) -> dict[str, Any]:
    ...
    if not resposta.success:
        raise HTTPException(status_code=400, detail="Não foi possível consultar a sessão online.")
